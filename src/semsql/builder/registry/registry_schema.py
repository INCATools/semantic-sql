...
class Ontology(YAMLRoot):
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if not isinstance(self.post_processing_steps, list):
            self.post_processing_steps = [self.post_processing_steps] if self.post_processing_steps is not None else []
        self.post_processing_steps = [v if isinstance(v, str) else str(v) for v in self.post_processing_steps]
...
slots.post_processing_steps = Slot(uri=SEMSQL_REGISTRY.post_processing_steps, name="post_processing_steps", curie=SEMSQL_REGISTRY.curie('post_processing_steps'),
                   model_uri=SEMSQL_REGISTRY.post_processing_steps, domain=None, range=Optional[Union[str, List[str]]])
